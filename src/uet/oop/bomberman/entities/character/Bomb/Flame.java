...
public class Flame extends Entity {
            ...
            switch (movingState) {
                case UP -> {
                    for (int i = 0; i < length - 1; i++) {
                        gc.drawImage(bImage, x, y - SCALED_SIZE * i);
                    }
                ...
                case LEFT -> {
                    for (int i = 0; i < length - 1; i++) {
                        gc.drawImage(bImage, x - SCALED_SIZE * i, y);
                    }
    ...
    public void update() {
        ...
        if (length >= 2) {
            switch (movingState) {
                case UP, DOWN -> this.bImage = Sprite.movingSprite(Sprite.explosion_vertical, Sprite.explosion_vertical1,
                        Sprite.explosion_vertical2, explodingTime, FPS).getFxImage();
                case LEFT, RIGHT -> this.bImage = Sprite.movingSprite(Sprite.explosion_horizontal, Sprite.explosion_horizontal1,
